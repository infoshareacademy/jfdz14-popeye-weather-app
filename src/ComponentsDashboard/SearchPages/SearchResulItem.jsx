...
class SearchResultItem extends React.Component {
    ...
    addStorage = () => {
        ...
        toggleFavourites(toState, this.long, this.lat, this.cityDetails.name)
            .then(x => {
    ...
    get cityDetails() {
        return getCities().find(
            city => city.long === this.long && city.lat === this.lat
        );
    }
    render() {
        ...
        return (
            <AppContent>
                {(cityDetails && weatherData) ? this.renderCityDetails(cityDetails, weatherData, isFavourite) : null}
            </AppContent>
        );
    }
    ...
    renderCityDetails(cityDetails, weather, isFavourite) {
        return <>
            ...
            <div className={style.metadata}>
                <MetadataEntry name="Temperature">{(weather.temperature - 273.15).toFixed(0)} ℃</MetadataEntry>
                ...
                <MetadataEntry name="Precipitation">{precipitationDescription(weather.precipitation)}</MetadataEntry>
            </div>
        </>;
    }
}
...
function MetadataEntry(props) {
    ...
    return <div className={style.metadataEntry} {...otherProps}>
        <dt className={style.metadataEntryTitle}>
            {name}
        </dt>
        <dd className={style.metadataEntryContent}>
            {children}
        </dd>
    </div>;
}
